...
# SMB Diagnostic Tool for macOS
# Maintained by scavenger503 & World of Hackers LLC
...
check_tools() {
    ...
    if ! command -v mount_smbfs &>/dev/null; then
        log ERROR "mount_smbfs is missing. SMB client support not found."
        exit 1
    fi
    if ! command -v nc &>/dev/null; then
        log ERROR "nc (netcat) is missing. Install it using: brew install netcat"
        exit 1
    fi
...
check_remote_smb_port() {
    log INFO "Checking connectivity to $SMB_SERVER on port 445..."
    if nc -z -G 3 "$SMB_SERVER" 445; then
        log OK "Port 445 is open."
    else
        log ERROR "Cannot reach port 445 on $SMB_SERVER."
        echo -e "Ensure the server is reachable and port 445 is open.\n"
        exit 1
    fi
}

check_authentication() {
    log INFO "Attempting to mount the SMB share..."
    local mount_point="/Volumes/SMBTest_$(date +%s)"
    mkdir -p "$mount_point"

    if [[ -n "$SMB_USER" && -n "$SMB_PASS" ]]; then
        URL="smb://$SMB_USER:$SMB_PASS@$SMB_SERVER/$SMB_SHARE"
    else
        URL="smb://$SMB_SERVER/$SMB_SHARE"
    fi

    if mount_smbfs "$URL" "$mount_point" 2>/dev/null; then
        log OK "Mounted successfully at $mount_point"
        if ls "$mount_point" >/dev/null; then
            log OK "Read access confirmed."
        else
            log WARN "Mount succeeded, but read access failed."
        fi
        umount "$mount_point" && rmdir "$mount_point"
        log OK "Unmounted and cleaned up."
    else
...
check_remote_smb_port
check_authentication
echo -e "\n${GREEN}SMB diagnostic complete.${NC}\n"
