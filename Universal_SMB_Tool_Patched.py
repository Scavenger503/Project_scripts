...
class SMBDiagnosticTool:
    ...
    def mount_macos_smb(self, server, share, username, password, mount_point):
        ...
        try:
            ...
            # Encode the share name to handle spaces
            encoded_share = share.replace(' ', '%20')

            # Escape '@' in credentials to prevent URL parsing errors
            username_safe = username.replace('@', '%40') if username else ''
            password_safe = password.replace('@', '%40') if password else ''

            # Build SMB URL
            if username and password:
                smb_url = f"smb://{username_safe}:{password_safe}@{server}/{encoded_share}"
            else:
                smb_url = f"smb://{server}/{encoded_share}"
